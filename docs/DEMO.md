...
`verify:demo` also runs the Research Agent against procurement and confirms the task is denied with `ROLE_MISMATCH` without any `POLICY_CHECKING` activity. Revocation remains an explicit admin-wallet demo step because the verifier must not mutate ENS state automatically.
...
3. Research Agent + procurement request: expected ENS role denial before policy.
