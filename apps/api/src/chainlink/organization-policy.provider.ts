...
export class OrganizationPolicyProvider implements ConfidentialPolicyProvider {
  ...
  async evaluate(input: PolicyEvaluationInput): Promise<PolicyEvaluationResult> {
    ...
    const provider =
      ...
      (this.config.get("POLICY_PROVIDER", { infer: true }) === "chainlink"
        ? "chainlink"
        : "manual");
    if (this.config.get("HACKATHON_MODE", { infer: true }) && provider !== "chainlink") {
      throw new ServiceUnavailableException(
        "Hackathon mode requires Chainlink confidential policy",
      );
    }
    return provider === "chainlink"
      ? this.chainlink.evaluate(input)
      : this.manual.evaluate(input);
  }
}
