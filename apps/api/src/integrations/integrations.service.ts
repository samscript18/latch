...
import { lastIntegrationSuccess, markIntegrationSuccess } from "../common/integration-telemetry.js";
...
export class IntegrationsService {
	...
	async status() {
		...
		return {
			...
			confidentialPolicy: {
				...
				state: policyProvider === "chainlink" && this.config.get("CRE_WORKFLOW_URL", { infer: true }) ? "configured" : policyProvider === "chainlink" ? "missing_configuration" : "local_development",
				validForHackathon: policyProvider === "chainlink" && Boolean(this.config.get("CRE_WORKFLOW_URL", { infer: true })),
				environment: this.config.get("CRE_ENVIRONMENT", { infer: true }) ?? null,
				lastSuccessfulSimulation: lastIntegrationSuccess("chainlink"),
			},
			capability: {
				provider: capabilityProvider,
				state: capabilityProvider === "recipe" && bazanticUrl && recipeId && bazanticApiKey ? "configured" : capabilityProvider === "recipe" ? "missing_configuration" : "local_development",
				validForHackathon: capabilityProvider === "recipe" && Boolean(bazanticUrl && recipeId && bazanticApiKey),
			...
			research: {
				provider: "tavily",
				state: this.config.get("TAVILY_API_KEY", { infer: true }) ? "configured" : "missing_configuration",
				validForHackathon: Boolean(this.config.get("TAVILY_API_KEY", { infer: true })),
				lastSuccessfulInvocation: lastIntegrationSuccess("tavily"),
			},
			planner: {
				provider: plannerProvider,
				state: plannerProvider === "gemini" ? "configured" : "local_development",
	...
	private async checkEns() {
		const name = this.config.get("DEMO_PROCUREMENT_AGENT_ENS", { infer: true }) ?? this.config.get("DEMO_ORG_ENS", { infer: true });
		...
		try {
			...
			return {
				...
				checkedName: name,
				lastSuccessfulResolution: resolved ? markIntegrationSuccess("ensv2") : lastIntegrationSuccess("ensv2"),
			};
		} catch {
			return {
				provider: "ensv2",
