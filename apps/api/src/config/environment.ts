...
export const EnvironmentSchema = z
	.object({
		...
		WEB_ORIGIN: z.url().default("http://localhost:3000").optional(),
		MONGODB_URI: z.string().min(1).default("mongodb://127.0.0.1:27017/latch"),
		AUTH_CHALLENGE_TTL_SECONDS: z.coerce.number().int().min(60).max(600).default(300),
		AUTH_SESSION_TTL_SECONDS: z.coerce.number().int().min(300).max(86_400).default(900),
		...
		ENSV2_UNIVERSAL_RESOLVER_ADDRESS: optionalAddress.default(ENSV2_HACKATHON_UNIVERSAL_RESOLVER_ADDRESS),
	...
	.superRefine((environment, context) => {
		if (environment.HACKATHON_MODE && environment.POLICY_PROVIDER !== "chainlink") {
			context.addIssue({
				code: "custom",
		...
		if (environment.HACKATHON_MODE && environment.POLICY_PROVIDER === "chainlink" && !environment.CRE_WORKFLOW_URL) {
			context.addIssue({
				code: "custom",
		...
		if (environment.HACKATHON_MODE && environment.PLANNER_PROVIDER !== "gemini") {
			context.addIssue({
				code: "custom",
		...
		if (environment.PLANNER_PROVIDER === "gemini" && !environment.GOOGLE_CLOUD_API_KEY) {
			context.addIssue({
				code: "custom",
		...
		if (environment.HACKATHON_MODE && environment.CAPABILITY_PROVIDER !== "recipe") {
			context.addIssue({
				code: "custom",
		...
		if (environment.HACKATHON_MODE && environment.CAPABILITY_PROVIDER === "recipe" && (!environment.BAZANTIC_GATEWAY_URL || !environment.BAZANTIC_RECIPE_ID || !environment.BAZANTIC_API_KEY)) {
			context.addIssue({
				code: "custom",
		...
		if (environment.SEPOLIA_CHAIN_ID !== 11_155_111) {
			context.addIssue({
				code: "custom",
				message: "LATCH chain integrations must target Sepolia (chain ID 11155111)",
