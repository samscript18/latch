		...
		<main className="dashboard-shell">
			<section className="dashboard-heading">
				<div>
					...
					<h1>{organization.data?.name ?? "Workspace"}</h1>
					<p className="muted dashboard-namespace">{organization.data?.ensName ?? "ENS namespace pending configuration"}</p>
				</div>
				<div className="integration-pills">
					...
					<IntegrationPill label="Bazantic" value={integrations.data?.capability.state} />
			...
			<section className="try-now-banner">
				<div>
					<p className="eyebrow">Authorization workspace</p>
					<h2>Put an AI worker through the complete LATCH pipeline.</h2>
					<p>Propose an action, verify its live ENS identity, evaluate organizational policy, and release only authorized execution.</p>
				</div>
				<Link className="brand-button" href="/app/try">
					Try LATCH now <span aria-hidden="true">→</span>
				</Link>
			</section>
						...
						return (
							<article className="agent-card" key={agent.id}>
								...
								<dl className="agent-facts">
									<Fact label="Wallet" value={shortAddress(agent.identity?.wallet)} />
									<Fact label="Role" value={agent.identity?.role ?? "Not verified"} />
									<Fact label="Capability" value={agent.identity?.capabilities.join(", ") || "None"} />
								</dl>
			...
			<section className="panel activity-panel">
				<div className="section-heading">
					<div>
						...
						<h2>Recent authorization events</h2>
					</div>
				...
				<div className="activity-list">
					{organization.data?.recentActivity.length ? (
						...
						<p className="muted empty-copy">No authorization attempts recorded yet.</p>
					)}
				</div>
			</section>
			...
			{revokeTarget && (
				<div className="dialog-backdrop" role="presentation" onMouseDown={() => setRevokeTarget(null)}>
					<section className="dialog" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
						<p className="eyebrow danger-eyebrow">Authority revocation</p>
						<h2>Revoke {revokeTarget.displayName}?</h2>
						<p className="muted dialog-copy">This writes the revoked status to ENS and prevents future LATCH-authorized execution.</p>
						{revoke.error && <div className="notice error">{revoke.error.message}</div>}
						<div className="dialog-actions">
							<button className="brand-button-secondary" onClick={() => setRevokeTarget(null)}>
								Cancel
							</button>
							<button className="button-danger" disabled={revoke.isPending} onClick={() => revoke.mutate(revokeTarget)}>
								{revoke.isPending ? "Revoking…" : "Revoke on ENSv2"}
							</button>
						</div>
					</section>
				</div>
...
function IntegrationPill({ label, value }: { label: string; value?: string }) {
	...
	return (
		<span className={`integration-pill ${active ? "integration-configured" : ""}`}>
			<i />
			{label}: {value?.replaceAll("_", " ") ?? "checking"}
		</span>
	);
}
