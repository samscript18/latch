	...
	if (task.error)
		return (
			<main className="detail-shell app-page">
				<div className="notice error">{task.error.message}</div>
			</main>
		);
	...
	return (
		<main className="detail-shell app-page">
			<header className="detail-header">
				<div>
					<Link className="back-link" href="/app/tasks">
						← All tasks
					</Link>
					<p className="eyebrow">Task {task.data.task.id.slice(-8)}</p>
					<h1>{task.data.task.prompt}</h1>
					<p className="muted">
						{agent?.displayName ?? "AI worker"} · {agent?.ensName ?? task.data.task.agentId}
					</p>
				</div>
					...
					{actionEntries.length ? (
						<dl>
							{actionEntries.map(([key, value]) => (
								<Fact key={key} label={humanize(key)} value={formatValue(value)} />
							))}
						</dl>
					) : (
				...
				<section className="panel task-timeline">
					<div className="section-heading">
						<div>
							...
							<h2>Sourced products</h2>
						</div>
					</div>
					<div className="procurement-result-summary">
						{procurementResults.map((product) => (
							<article key={product.productId ?? product.name}>
								<div>
									...
									<div>
										...
										<small>
											{product.vendor} · {humanize(product.source ?? "capability provider")}
										</small>
									</div>
								</div>
								<dl>
			...
			<section className="panel task-timeline">
				<div className="section-heading">
					<div>
						<p className="eyebrow">Authorization trail</p>
						<h2>Task activity</h2>
					</div>
				</div>
			...
			<div className="task-danger-zone">
				<div>
					...
					<p>Hide this task while retaining its security audit evidence.</p>
				</div>
				...
				<div className="dialog-backdrop" role="presentation" onMouseDown={() => setConfirmingDelete(false)}>
					<section className="dialog" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
						...
						<div className="dialog-actions">
							<button className="brand-button-secondary" onClick={() => setConfirmingDelete(false)}>
								Cancel
							</button>
							<button className="button-danger" disabled={remove.isPending} onClick={() => remove.mutate()}>
...
function readProcurementResults(value: unknown): Array<{ productId?: string; name: string; vendor: string; productUrl?: string; quantity: number; unitPriceCents: number; totalAmountCents: number; source?: string }> {
	...
	return value.filter((item): item is { productId?: string; name: string; vendor: string; productUrl?: string; quantity: number; unitPriceCents: number; totalAmountCents: number; source?: string } => {
		...
		return (
			...
			typeof product.totalAmountCents === "number" &&
			(product.productId === undefined || typeof product.productId === "string") &&
			(product.productUrl === undefined || typeof product.productUrl === "string") &&
